...
import { getViewModeVisibility, useViewMode, type ViewMode } from './hooks/useViewMode'
...
const RESEARCH_LAB_DOMAIN_KEYS: readonly ResearchLabDomainKey[] = [
  'ongoingProjects',
  'projectAcquisition',
  'teaching',
  'labManagement',
...
function resolveSelectedLabDomain(
  ...
  for (const key of RESEARCH_LAB_DOMAIN_KEYS) {
...
function App() {
  ...
  const researchLabModeEnabled = vaultConfig.researchLabMode?.enabled === true
  ...
  const handleSetViewMode = useCallback((mode: ViewMode) => {
    setViewMode(mode)
    const nextVisibility = getViewModeVisibility(mode)
    updateMainWindowConstraints(nextVisibility.sidebarVisible, nextVisibility.noteListVisible)
  ...
  const handleCollapseNoteList = useCallback(() => {
    handleSetViewMode(sidebarVisible ? 'editor-sidebar' : 'editor-only')
  }, [handleSetViewMode, sidebarVisible])

  const handleExpandNoteList = useCallback(() => {
    handleSetViewMode(sidebarVisible ? 'all' : 'editor-list')
  }, [handleSetViewMode, sidebarVisible])
  ...
  const isLabHomeSelection = effectiveSelection.kind === 'filter' && effectiveSelection.filter === 'labHome'

  const aiNoteList = useMemo<NoteListItem[]>(() => {
    const isLabHome = effectiveSelection.kind === 'filter' && effectiveSelection.filter === 'labHome'
    const isInbox = effectiveSelection.kind === 'filter' && effectiveSelection.filter === 'inbox'
    const filtered = isLabHome
      ? []
      : isInbox
